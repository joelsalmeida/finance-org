...
describe('Money', () => {
  describe('fromCents', () => {
    it('fails to create Money when amount is Infinity', () => {
      expect(() => Money.fromCents(Infinity)).toThrow(
        InvalidMoneyAmountException
      );
    });

    it('fails to create Money when amount is NaN', () => {
    ...
    it('creates a Money instance with zero amount', () => {
  ...
  describe('isZero', () => {
    it('returns true when the amount is zero', () => {
      const money = Money.fromCents(0);

      expect(money.isZero()).toBe(true);
    });

    it('returns false when the amount is greater than zero', () => {
  ...
  describe('isLessThan', () => {
    it('returns true when the amount is smaller', () => {
      const smaller = Money.fromCents(100);
      const larger = Money.fromCents(200);

      expect(smaller.isLessThan(larger)).toBe(true);
    });

    it('returns false when the amount is greater', () => {
      const larger = Money.fromCents(300);
      const smaller = Money.fromCents(100);
    ...
    it('returns false when amounts are equal', () => {
      const money1 = Money.fromCents(200);
      const money2 = Money.fromCents(200);
  ...
  describe('isGreaterThan', () => {
    it('returns true when the amount is greater', () => {
      const larger = Money.fromCents(500);
      const smaller = Money.fromCents(200);

      expect(larger.isGreaterThan(smaller)).toBe(true);
    });

    it('returns false when the amount is smaller', () => {
      const smaller = Money.fromCents(100);
      const larger = Money.fromCents(300);
    ...
    it('returns false when amounts are equal', () => {
      const money1 = Money.fromCents(400);
      const money2 = Money.fromCents(400);
